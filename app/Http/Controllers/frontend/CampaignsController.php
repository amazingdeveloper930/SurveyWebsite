...
class CampaignsController extends Controller
{
    ...
    public function setCampaignlanguageAccordingtoUser()
    {
        $mode = 3;
        if($mode == 1){
            $entries = Campaign::all();
        ...
        if($mode == 2){
            ...
            foreach ($entries as $entry) {
                $array = array();
                $array [0]= $entry -> language;
                if(isset($entry -> language)){
                    $entry -> language_list = serialize($array);
                    $entry -> save();
                }
...
// Total user credits
        ...
        $entries = Campaign::where('user_id', Auth::guard('web')->user()->id)

            ->orderBy('advertise_credits', 'desc')
            ->paginate(20);

        foreach ($entries as $key => $value) {
            $entriesnew = CampaignResult::select('user_id')->where('updated_at','>=' , date('Y-m-d',strtotime(Auth::guard('web')->user()->last_login)))->where('campaign_id', '=', $value->id)->get();

            $entriestotal = CampaignResult::select('user_id')->where([['campaign_id', '=', $value->id]])->get();

            $value->newrespondents = $entriesnew->unique('user_id')->count();
            $value->totalrespondents = $entriestotal->unique('user_id')->count();
            $entries[$key]         = $value;
        }
...
//CampaignResult::where('campaign_id', '=', id )->count()

        /*$anketos = Campaign::where('active', '=', 1)
        ->where('advertise_credits', '>', 0)
        ->where('public', '=', 1)
        ->orderBy('advertise_credits', 'desc')
        ->paginate(20);*/
        ...
        {
             $anketos = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id)
        ...
        /*$anketos = DB::table('campaigns AS c')
        ->select('*')
        ->where('advertise_credits', '>', 0)
        ->where('public', '=', 1)
        ->orderBy('advertise_credits', 'desc');*/
    ...
    public function handleFacebookShare($id){
        
        try{ 
            $message = '"hello": title
            "how are you": text
            "survey id":'. $id;
            Share::page($message)->facebook();
        }catch(\Exception $e){
            echo $e->getMessage();
        }
    ...
    public function reportFacebookShare($id){
        ...
        {
            Campaign::where('id', $id)->update(array('shared' => 2, 'shared_at' => date('Y-m-d H:i:s')));

            $campaign = Campaign::find($id);
            $credits = new UserCredit;
            $credits->user_id     = $campaign->user_id;
            $credits->description = 'For Facebook Share';
    ...
    public function answer($id, Request $request)
    {
        ...
        if ($entry && $entry->active) {
            ...
            if ($available) {
               ...
               if (Auth::check()) {
                     $entries = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND id not in (SELECT 
                        distinct campaign_id
                    FROM
                        campaigns_results AS cr
                    WHERE
                        cr.user_id = '. Auth::guard('web')->user()->id .') AND id != '.$entry->id.' AND c.public = 1 AND c.active = 1 ORDER BY advertise_credits DESC LIMIT 10');
                    $top = 'frontend.campaigns.survey_destails';

                } else {
                    $entries = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND id != '.$entry->id.' AND public = 1 AND active = 1 ORDER BY advertise_credits DESC LIMIT 20');
                    $top = 'frontend.campaigns.survey_fdetails';
                }
                return view($top, [
                    'entry'   => $entry,
                    'entries' => $entries,
                ...
                Session::flash('message', "Success! You answered questions to this survey.");
    ...
    public function store_answer($id, Request $request)
    {
        ...
        if ($campaign && $campaign->active) {
            if (!Auth::guard('web')->check() || Auth::guard('web')->user()->id !== $campaign->user_id) {
                ...
                if ($data->fails()) {
                    return response()->json($data->errors(), 422); //i'm not getting any json with errors
                }

                // Featured
                if ($campaign->advertise_results == 1) {
                    $price             = Credits::credits('featured_credits');
                    ...
                    $credits->user_id     = $campaign->user_id;
                    $credits->description = 'For promotion of survey';
                    $credits->credits     = $price;
                    ...
                    // Price
                    $price = Credits::credits('campaigns_credits');
                    ...
                    if ($credits > 0 && ($credits - $price) >= 0) {
                        ...
                        $credits->user_id     = $campaign->user_id;
                        $credits->description = 'For answer';
                        $credits->credits     = $price;
                ...
                if ($campaign->respondents) {
                    ...
                    Session::flash("message", "You have answered questions of this survey.");
                    return redirect()->route('pages.all-serveys');
                }
            }
    ...
    private function store_results($id, $campaign, $request, $price)
    {
        ...
        // print_r($request->get('i_683'));
        // Store question
        foreach ($campaign->questions as $question) {
            if ($request->has('i_' . $question->id)) {
                if (in_array($question->type, ['radio', 'select', 'string', 'text'])) {
                    ...
                    if (in_array($question->type, ['radio', 'select'])) {
                        $answer->option_id = $request->get('i_' . $question->id);
                    }

                    if (in_array($question->type, ['string', 'text'])) {
                        $answer->value = $request->get('i_' . $question->id);
                    }
                    ...
                    if ($question->type == 'check') {
                        foreach ($request->get('i_' . $question->id) as $option_id => $value) {
                            $answer = new CampaignAnswer;
    ...
    public function create()
    {
        ...
        // 
        $entries = null;
        if (Auth::check() && isset(Auth::guard('web')->user()->language_list) && Auth::guard('web')->user()->language_list != '') {
            $language_list = unserialize(Auth::guard('web')->user()->language_list);
            $entries = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND id not in (SELECT 
                distinct campaign_id
            FROM
                campaigns_results AS cr
            WHERE
                cr.user_id = '.Auth::guard('web')->user()->id.') AND c.public = 1 AND c.active = 1 
                AND ( c.language in ('.implode(",",$language_list).'))
                ORDER BY advertise_credits DESC LIMIT 10');
        }
        else{
            $entries = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND public = 1 AND active = 1 ORDER BY advertise_credits DESC LIMIT 20');
        }
    ...
    public function store(Request $request)
    {
        ...
        $entry->same_computer = $request->same_computer ? 1 : 1;
...
//$entry->id
        //return redirect("https://pollanimal.com/survey/".$entry->id."/settings");
        return redirect()->route('campaigns.edit', $entry->id);
    }
    ...
    public function edit($id)
    {
        ...
        if ($entry && $entry->user_id == Auth::guard('web')->user()->id) {
            ...
            /* $entriesnew = Campaign::where('active', '=', 1)
            ->where('public', '=', 1)
            ->orderBy('advertise_credits', 'desc')
            ->paginate(20);*/
            // $entriesnew = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND public = 1 AND active = 1 ORDER BY advertise_credits DESC LIMIT 20');
            $entriesnew = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND id not in (SELECT 
            distinct campaign_id
        FROM
            campaigns_results AS cr
        WHERE
            cr.user_id = '.Auth::guard('web')->user()->id.') AND c.public = 1 AND c.active = 1 ORDER BY advertise_credits DESC LIMIT 10');

            $questions = CampaignQuestion::where('campaign_id', '=', $entry->id)->get();
    ...
    public function answers($id)
    {
        ...
        if ($entry) {
            if ($entry->respondents) {
                ...
                if (Auth::check()) {
                    $anketos = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND id not in (SELECT 
                    distinct campaign_id
                    FROM
                        campaigns_results AS cr
                    WHERE
                        cr.user_id = '.Auth::guard('web')->user()->id.') AND id != '.$entry->id.' AND c.public = 1 AND c.active = 1 ORDER BY advertise_credits DESC LIMIT 10');
                    // print_r(array(Auth::guard('web')->user()->id,$anketos));exit();
                    $top = "frontend.results.index";
                } else {
                    $anketos = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND id != '.$entry->id.' AND public = 1 AND active = 1 ORDER BY advertise_credits DESC LIMIT 10');
                    $top = "frontend.results.findex";
                }
    ...
    public function update($id, Request $request)
    {
        ...
        if ($entry && $entry->user_id == Auth::guard('web')->user()->id) {
            ...
            /* echo "available_credits : ".$available_credits.'<br>';
            echo "price : ".$price.'<br>';
            echo "advertise_results : ".$request->advertise_results.'<br>';die(); */

            $validation = Validator::make($request->all(), $rules);
            // \Log::info(print_r($validation->errors(),true));
            ...
            if ($validation->fails()) {
                ...
                if ($request->btn_launch) {
                    $request->active = $request->btn_launch;
                    $request->public = 1;
                }

                if ($request->btn_public) {
                    $request->public = 1;
                    $request->active = 1;
                }

                $entry->user_id       = Auth::guard('web')->user()->id;
                $entry->title         = isset($request->title) ? $request->title : $entry->title;
                $entry->description   = isset($request->description) ? $request->description : $entry->description;
                $entry->video         = $request->video;
                $entry->respondents   = $request->respondents ? 1 : 0;
                $entry->send_email    = $request->send_email ? 1 : 0;
                ...
                // Added validation to ensure credits gets added only when available credits is greater then what was requested to add
                if ($request->advertise_results > 0) {
                    if( $available_credits > $request->advertise_results ){
                        $entry->advertise_credits = $entry->advertise_credits + $request->advertise_results;
                    }
                    elseif( $available_credits == $request->advertise_results ){
                        $entry->advertise_credits = $entry->advertise_credits + $available_credits;
                    }
                    else{
                        $entry->advertise_credits = $entry->advertise_credits + $available_credits;
                        $request->advertise_results = $available_credits;
                    }
                }
                ...
                if(count($questions) >= 1){
                    ...
                    Session::flash('message', "Please add minimum of one question to your survey.");
                    return redirect()->back();
                }
                ...
                if ($request->advertise_results > 0) {
                    UserCredit::create([
                        ...
                        'description' => 'For promotion of survey',
                    ]);
                    Session::flash('message', "Credit was successfully added for the promotion of survey.");
                }
            }
